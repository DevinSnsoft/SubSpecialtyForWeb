...
/**
 * Servlet implementation class ControllInfo
 */
@WebServlet("/ControllInfo")
public class ControllInfo extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		if (StringUtils.isEmpty(param.get("id"))) {
			...
			try {
				...
				HashMap<String, Object> res = ad.getControllInfo(param);
				if (res != null) {
					result = JsonUtils.jsonResponse(res,
							AllConstant.CODE_SUCCESS, AllConstant.MSG_SUCCESS);
				} else {
					result = JsonUtils.jsonResponse(null,
							AllConstant.CODE_ERROR, "该控制信息不存在！");
				}
